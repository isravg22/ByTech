...
export default function Smartphone({navigation}:any) {
    ...
    const getProducts = async () => {
        try {
            const response = await fetch('http://192.168.0.247:8001/product/type/Smartphones', {
                method: 'GET',
                headers: {
                ...
                {smartphones.map((smartphone) => (
                    <TouchableOpacity key={smartphone.id} style={{ backgroundColor: '#ffffff', marginBottom: 20, borderWidth: 1, borderColor: '#ccc', borderRadius: 8 }}>
                        <View style={{ height: 200, overflow: 'hidden' }}>
                            <Image source={{ uri: smartphone.image }} style={{ width: '100%', height: '100%', resizeMode: 'cover' }} />
                        </View>
