...
export default function Smartphone(){
  ...
  return (
    <div>
      <NavBar />
      <h1 style={{ marginBottom: '20px', textAlign: 'center', marginTop: '5%', fontSize: '30px', fontWeight: 'bold' }}>NUESTROS COMPONENTES</h1>
      {loading && <p>Cargando productos...</p>}
      {error && <p>{error}</p>}
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '20px', }}>
        {componentes.map((componente) => (
          <div key={componente.id} style={{ flex: '0 1 23%', padding: '20px', marginBottom: '40px', border: '1px solid #ccc', borderRadius: '8px', backgroundColor: '#fff', textAlign: 'center', marginLeft: '1%', marginRight: '1%' }}>
            <div style={{ marginBottom: '20px', width: '100%', height: '200px', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
              <img src={componente.image} alt={componente.name} style={{ width: '40%', height: '100%' }} />
            </div>
            <div style={{ textAlign: 'left' }}>
              <h2 style={{ margin: '0 0 10px 0', fontSize: '1.2rem', fontWeight: 'bold' }}>{componente.name.toUpperCase()}</h2>
              ...
              <p style={{ margin: '0 0 10px 0', fontSize: '1rem' }}>Precio: {componente.price}€</p>
              <div onClick={() => handleDetailClick(componente.id)}>
