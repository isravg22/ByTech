...
import cartStyle from '@/Component/Cart/cart.module.css';
...
export default function Cart() {
    ...
    const closeFeedback = (event:any, reason:any) => {
        if (reason === 'clickaway') {
            return;
        }
        setOpen(false);
            ...
            <Grid container spacing={1} className={cartStyle.grid}>
                <Grid item xs={12} md={8} justifyContent='center'>
                    <div className={cartStyle.title}>
                        <Typography  ml={2} color={'#1976d2'} fontSize={40} component="h2" fontWeight={500}>
                            Mi carrito
                        </Typography>
                    </div>
                    <Stack className={cartStyle.cart_container} alignItems='center' >
                        {productList.map(item =>
                            <React.Fragment key={item}>
                                <Grid container spacing={2} mt={1} mb={1} justifyContent='center' alignItems={'center'}>
                                    ...
                                    <Grid item xs={6} md={3}>
                                        <Typography  fontSize={20} component="h2" fontWeight={500}>
                                            {item.product.name}
                                        </Typography>
                                    </Grid>
                                    <Grid item xs={2} md={2}>
                                    ...
                                    <Grid item xs={9} md={2}>
                                        <Typography   fontSize={24} component="h2" fontWeight={700}>
                        ...
                        <div className={cartStyle.total}>
                            {productList.length ?
                                <>
                                    <Typography   mr={4} fontSize={24} component="h2" fontWeight={700}>
                ...
                <Grid item xs={12} md={3} justifyContent='center'>
                    <div className={cartStyle.prev_container}>
                        <Typography   color={'#1976d2'} fontSize={30} component="h2" fontWeight={500}>
                            Mis Compras
                        </Typography>
                        {
                            salesList.map(sale =>
                                <div className={cartStyle.prev_sales} key={sale}>
                                    <Typography   fontSize={20} component="h2" fontWeight={500}>
                                        Fecha: {/* {getDate(sale.date)} */}
                                    </Typography>
                                    <Typography mt={2}   fontSize={25} component="h2" fontWeight={600}>
